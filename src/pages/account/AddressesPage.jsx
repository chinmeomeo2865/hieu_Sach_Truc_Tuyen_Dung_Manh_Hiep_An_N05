...
function AddressForm({ initial, onSave, onCancel, loading }) {
  ...
  const INPUT = 'w-full border border-[#e5e5e5] rounded-lg px-3 py-2 text-[13.5px] text-[#0f0f0f] placeholder:text-[#a3a3a3] focus:outline-none focus:border-[#0f0f0f] transition-colors'
  const ERROR = 'text-[11px] text-red-500 mt-1'

  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#737373] mb-1.5">Nhãn (tùy chọn)</label>
        <input value={form.label} onChange={set('label')} placeholder="VD: Nhà, Văn phòng…" className={INPUT} />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#737373] mb-1.5">Họ tên người nhận *</label>
          <input value={form.name} onChange={set('name')} placeholder="Nguyễn Văn A" className={INPUT} />
          {errors.name && <p className={ERROR}>{errors.name}</p>}
        </div>
        <div>
          <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#737373] mb-1.5">Số điện thoại *</label>
          <input value={form.phone} onChange={set('phone')} placeholder="0383 687 670" className={INPUT} />
          {errors.phone && <p className={ERROR}>{errors.phone}</p>}
        </div>
      </div>
      <div>
        <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#737373] mb-1.5">Địa chỉ (số nhà, đường) *</label>
        <input value={form.street} onChange={set('street')} placeholder="123 Đường Nguyễn Trác" className={INPUT} />
        {errors.street && <p className={ERROR}>{errors.street}</p>}
      </div>
      <div>
        <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#737373] mb-1.5">Tỉnh / Thành phố</label>
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={loading}
          className="px-5 py-2 bg-[#0f0f0f] text-white text-[13px] font-medium rounded-lg hover:bg-[#3d3835] disabled:opacity-50 transition-colors">
          {loading ? 'Đang lưu…' : 'Lưu địa chỉ'}
        </button>
        <button type="button" onClick={onCancel}
          className="px-5 py-2 border border-[#e5e5e5] text-[#404040] text-[13px] font-medium rounded-lg hover:border-[#a3a3a3] transition-colors">
          Hủy
        </button>
      </div>
    </form>
...
export default function AddressesPage() {
  ...
  if (loading) return (
    <div className="max-w-[640px] mx-auto px-4 py-16 text-center text-[13px] text-muted">Đang tải…</div>
  )
  ...
  return (
    <div className="max-w-[640px] mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink">Địa chỉ giao hàng</h1>
          <p className="text-[13px] text-muted mt-1">Quản lý địa chỉ nhận hàng của bạn</p>
        </div>
        {!mode && (
          <button onClick={() => setMode('add')}
            className="flex items-center gap-1.5 px-4 py-2 bg-ink text-white text-[13px] font-medium rounded-lg hover:bg-ink-80 transition-colors">
      ...
      {/* Form thêm/sửa */}
      {mode && (
        <div className="bg-[#fafaf9] border border-divider-lt rounded-xl p-5 mb-6">
      ...
      {addresses.length === 0 && !mode ? (
        <div className="text-center py-16 text-[13px] text-muted">
          ...
          Chưa có địa chỉ nào. Thêm để checkout nhanh hơn!
        </div>
      ) : (
        <div className="space-y-3">
          {addresses.map(addr => (
            <div key={addr._id}
              className={`border rounded-xl p-4 transition-colors ${addr.isDefault ? 'border-ink bg-white' : 'border-divider-lt bg-white'}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {addr.isDefault && (
                      <span className="text-[10px] font-semibold uppercase tracking-wider bg-ink text-white px-2 py-0.5 rounded">
                        Mặc định
                      </span>
                    )}
                  </div>
                  <p className="text-[13.5px] font-medium text-ink">{addr.name}</p>
                  <p className="text-[13px] text-muted">{addr.phone}</p>
                  <p className="text-[13px] text-[#404040] mt-0.5">{addr.street}, {addr.city}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {!addr.isDefault && (
                    <button onClick={() => handleSetDefault(addr._id)}
                      className="text-[11.5px] text-muted hover:text-ink px-2 py-1 rounded hover:bg-[#f5f5f4] transition-colors">
                      Đặt mặc định
                    </button>
                  )}
                  <button onClick={() => setMode({ editing: addr })}
                    className="p-1.5 rounded hover:bg-[#f5f5f4] text-muted hover:text-ink transition-colors">

...
export function SectionHeader({ eyebrow, title, subtitle, linkText, linkHref = '#', titleHref }) {
    ...
    <div className="flex items-end justify-between gap-4 mb-8 md:mb-10">
      <div>
        ...
        <h2 className="font-display font-semibold text-2xl md:text-3xl leading-tight text-ink">
          {titleHref ? (
            <Link to={titleHref} className="transition-colors hover:text-muted">{title}</Link>
          ) : title}
        </h2>
        {subtitle && (
